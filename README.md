...
**Tools and build effort.** n8n Webhook/Code/HTTP Request, Node.js built-in HTTP server and SQLite. Built and tested in one working session (under one day), using synthetic contact data.
...
**Scope.** No commercial CRM, portal API, or external messaging service is connected in this sample; SQLite provides the persistence layer. In the pilot, this same service connects to the client's CRM through its API.
